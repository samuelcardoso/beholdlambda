...
export interface Route {
  ...
  handler: (event?: Partial<HandlerEvent>, context?: Context, cache?: any) => Promise<HandlerResponse>
}
...
export default class OpenAPIHandler {
  ...
  public swaggerEndpoint: string | undefined;
  ...
  public async handler(event: Partial<APIGatewayProxyEvent>, context?: Context, cache?: any): Promise<HandlerResponse> {
    const { path } = event;
    ...
    return this.route(event, context, cache);
  }

  // dank version of hapi's routing + joi validation
  private async route(event: Partial<APIGatewayProxyEvent>, context?: Context, cache?: any): Promise<HandlerResponse> {
    ...
    try {
      payload.payload = JSON.parse(body as any);
    } catch {
    ...
    const res = await handler({ ...event, ...payload }, context, cache);
    return {
      statusCode: 200,
      ...res
    };
