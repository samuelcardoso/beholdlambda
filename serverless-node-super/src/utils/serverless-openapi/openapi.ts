...
export default class OpenAPIBuilder {
  ...
  constructor(opts: OpenAPIBuilderOpts) {
    ...
    this.info = opts.info;
    this.servers = opts.servers as any;
    this.externalDocs = opts.externalDocs as any;
  ...
  private joiToOpenApiSchema(joi: Joi.SchemaLike) {
    const { swagger } = j2s(<Joi.Schema>joi);
  ...
  private nameToRef(name: string, context = '') {
